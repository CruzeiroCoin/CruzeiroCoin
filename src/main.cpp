...
#if defined(NDEBUG)
# error "Cruzeirocoin cannot be compiled without assertions."
#endif
...
uint256 hashGenesisBlock("0x05a77db8a4172be2c366a6db59baee5aa0fb036c99a06cb9e873cc8fb60bda89");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // Cruzeirocoin: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "Cruzeirocoin Signed Message:\n";
...
{
    // Cruzeirocoin: IsDust() detection disabled, allows any valid dust to be relayed.
...
int64 static GetBlockValue(int nHeight, int64 nFees)
{
    ...
    nSubsidy >>= (nHeight / 420000); // Cruzeirocoin: 420k blocks in ~2 years

    return nSubsidy + nFees;
}

static const int64 nTargetTimespan = 3 * 24 * 60 * 60; // Cruzeirocoin: 3 days
static const int64 nTargetSpacing = 2 * 60; // Cruzeirocoin: 2 minutes
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // Cruzeirocoin: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // Cruzeirocoin: Special short-term limits to avoid 10,000 BDB lock limit:
...
bool LoadBlockIndex()
{
    if (fTestNet)
    {
        ...
        hashGenesisBlock = uint256("0x6cf8f139281524f4dc411852828c29b0ff414059e8701de081cb9c98bb2cd9cd");
    }
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        // Genesis block
        const char* pszTimestamp = "NY Times 15/Oct/2021 Armeiro coin, Cruzeirocoin’s Visionary, Dies at 56";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        txNew.vin[0].scriptSig = CScript() << 486604799 << CBigNum(4) << vector<unsigned char>((const unsigned char*)pszTimestamp, (const unsigned char*)pszTimestamp + strlen(pszTimestamp));
        txNew.vout[0].nValue = 25 * COIN;
        txNew.vout[0].scriptPubKey = CScript() << ParseHex("048417f635f1d5437763e1c493ca5c6a84e45be34e1c0ec4fa18876a7c3e6be6a5b50e4cf74c55c9755e6fcafb313add65de9d608874af6b343ba8ed04253fa632") << OP_CHECKSIG;
        CBlock block;
        block.vtx.push_back(txNew);
        block.hashPrevBlock = 0;
        block.hashMerkleRoot = block.BuildMerkleTree();
        block.nVersion = 1;
        block.nTime    = 1632871963;
        block.nBits    = 0x1e0ffff0;
        block.nNonce   = 2084642120;

        if (fTestNet)
        {
            block.nTime    = 1632871892;
            block.nNonce   = 385637675;
        }
...
if (true && block.GetHash() != hashGenesisBlock)
        ...
        assert(block.hashMerkleRoot == uint256("0x4f6c600498dec8085b97fe66e7ee715523de8c77b6a6b166b9fa33e4667e40bf"));
        block.print();
...
unsigned char pchMessageStart[4] = { 0xfa, 0xc4, 0xb3, 0xda }; // Cruzeirocoin: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("CruzeirocoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("CruzeirocoinMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static CruzeirocoinMiner(CWallet *pwallet)
{
    printf("CruzeirocoinMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("cruzeirocoin-miner");
    ...
    try { loop {
        ...
        printf("Running CruzeirocoinMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("CruzeirocoinMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&CruzeirocoinMiner, pwallet));
}
